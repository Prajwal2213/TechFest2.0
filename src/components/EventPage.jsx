              ...
              <nav className="flex border-b border-zinc-800 shrink-0">
                {['SPECIFICATIONS', ].map((tab) => (
                  <button
                    ...
                    className={`flex-1 py-5 text-[10px] font-black tracking-[0.3em] transition-all relative ${
                      activeTab === tab ? 'text-white' : 'text-zinc-600 hover:text-zinc-400'
                    }`}
                  >
                    {tab}
                    {activeTab === tab && <div className="absolute bottom-0 left-0 w-full h-1 bg-yellow-500"></div>}
                  </button>
                ))}
              </nav>

              <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-8 scrollbar-custom">
                <div className="space-y-4">
                   <p className="text-zinc-300 text-lg md:text-xl leading-relaxed font-medium uppercase tracking-tight">
                    {event.description}
                   </p>
                </div>
...
export default function EventPage() {
  ...
  const refs = {
    robosoccer: useRef(null),
    drone: useRef(null),
    hack: useRef(null),
    techzibition: useRef(null),
    artistic: useRef(null),
        ...
        {Object.entries(eventData).map(([key, events]) => (
          <section key={key}>
            ...
            <div ref={refs[key]} className="flex gap-10 overflow-x-auto pb-16 pt-4 scrollbar-hide px-6 md:px-10 lg:px-16 scroll-smooth">
              ...
              <div className="w-80 shrink-0 border border-dashed border-zinc-800 flex flex-col items-center justify-center gap-4 opacity-30 mr-24">
              </div>
            </div>
          </section>
        ))}
