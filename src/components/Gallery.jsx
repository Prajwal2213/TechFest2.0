...
const CAROUSEL_DATA = [
  ...
  { id: 8, img: optimize("https://res.cloudinary.com/dstbnmjwh/image/upload/v1771940645/file_0000000051c871fd9db623250c475311_jc0kpz.png") },
...
export default function Gallery() {
  ...
  const renderItems = (data, reverse = false) =>
    data.filter(i => i.img).map((item, index) => (
      <div
        key={item.id}
...
.carousel-item.reverse {
  ...
  animation: marquee-reverse var(--carousel-duration) linear infinite;
      ...
      <div className="flex flex-col items-center text-center mt-25">
        <h1 className="text-5xl md:text-7xl lg:text-9xl font-Orbitron font-semibold tracking-wide uppercase leading-none text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-800 drop-shadow-2xl">
          Gallery
        </h1>
      </div>
      ...
      <div className="carousel-container mt-20">
        {renderItems(CAROUSEL_DATA)}
      </div>

      {/* Row 2 */}
      <div className="carousel-container mt-20">
        {renderItems(CAROUSEL_DATA_BOTTOM, true)}
      </div>

      {/* Row 3 */}
      <div className="carousel-container mt-20 mb-18">
        {renderItems(CAROUSEL_DATA_THIRD)}
      </div>
    </div>
  );
}