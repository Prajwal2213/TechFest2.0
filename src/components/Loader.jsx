...
const Loader = () => {
    ...
    <div className="relative w-full max-w-[560px] p-8 md:p-12 bg-[#040406]/94 border border-white/10 backdrop-blur-xl shadow-2xl overflow-hidden animate-shimmer pointer-events-auto"
        ...
        <div className={`text-white tracking-[0.6em] md:tracking-[1.2em] uppercase text-center mb-8 md:mb-12 text-xs md:text-sm font-bold opacity-90 transition-colors ${progress > 99 ? 'text-yellow-400' : 'animate-pulse'}`}>
            {mainLabel}
        </div>

        <div className="w-full h-3 md:h-4 bg-black/90 border border-white/15 relative overflow-hidden shadow-[inset_0_0_20px_rgba(0,0,0,1)]">
            <div 
        ...
        <div className="flex justify-between items-end mt-6">
            <div className="flex flex-col gap-1 pb-1 md:pb-2">
                <span className="text-[9px] md:text-[11px] font-bold tracking-[0.2em] md:tracking-[0.3em]" style={{ color: progress > 99 ? '#00ff88' : '#ffd700' }}>{phase}</span>
                <span className="text-[7px] md:text-[9px] opacity-40 font-mono">{hex}</span>
            </div>
            <div className="text-4xl md:text-6xl font-bold text-white leading-none">
                {progress}<span className="text-xl md:text-2xl ml-1 opacity-20 font-normal">%</span>
            </div>
        </div>

        {/* Tech Corners - Scaled slightly for mobile */}
        <div className="absolute top-0 left-0 w-10 h-10 md:w-16 md:h-16 border-t-4 border-l-4 border-yellow-400 drop-shadow-[0_0_15px_rgba(255,215,0,0.6)]" />
        <div className="absolute bottom-0 right-0 w-10 h-10 md:w-16 md:h-16 border-b-4 border-r-4 border-yellow-400 drop-shadow-[0_0_15px_rgba(255,215,0,0.6)]" />
    </div>
</div>
            )}
